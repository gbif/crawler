 ...
 */
package org.gbif.crawler.metasync.protocols.dwca;
...
/**
 * Moved to test package since this should not be used in production, EML are synchronized using
 * another path. It can be useful to run locally to test some changes though.
 *
 * <p>IPT and HTTP synchronization starts by iterating through all GBIF {@link Dataset} served by
 * the installation. Each GBIF {@link Dataset} is then augmented by its preferred metadata document,
 * typically an EML document.
 */
public class EmlMetadataSynchronizer implements MetadataProtocolHandler {

  private static final Logger LOG = LoggerFactory.getLogger(EmlMetadataSynchronizer.class);
  private final DatasetService datasetService;

  public EmlMetadataSynchronizer(DatasetService datasetService) {
