 ...
 */
package org.gbif.crawler.metasync;
...
/**
 * A concrete implementation of the {@link MetadataSynchronizer} interface. It delegates the actual
...
public class MetadataSynchronizerImpl implements MetadataSynchronizer {
  ...
  private static final Logger LOG = LoggerFactory.getLogger(MetadataSynchronizerImpl.class);
  ...
  public MetadataSynchronizerImpl(InstallationService installationService) {
    this.installationService = installationService;
  }

  @Override
  public SyncResult synchronizeInstallation(UUID key) {
    ...
    for (MetadataProtocolHandler protocolHandler : protocolHandlers) {
      if (protocolHandler.canHandle(installation)) {
        return doSynchronizeInstallation(installation, hostedDatasets, protocolHandler);
      }
    }
  ...
  @Override
  public List<SyncResult> synchronizeAllInstallations() {
    return synchronizeAllInstallations(1);
  }

  @Override
  public List<SyncResult> synchronizeAllInstallations(int parallel) {
        ...
        completionService.submit(
            new Callable<SyncResult>() {
              ...
              public SyncResult call() throws Exception {
                try {
                  return synchronizeInstallation(installation.getKey());
                } catch (Exception e) {
    ...
    while (!executor.isTerminated()) {
      LOG.info("Waiting for synchronizations to finish");
      try {
  ...
  /**
   * This method actually runs the metadata synchronization by calling out to the protocol handler
   * and then validating and processing its result.
   */
  private SyncResult doSynchronizeInstallation(
  ...
  /**
   * Does some checks whether we can synchronize this Installation or not. They are not exhaustive
