...
* [CRAwling Monitor (CRAM)](crawler-cleanup#crawling_monitor_(cram))
* [Kibana]()
* [RabbitMQ (Queues)]()
* [Registry Console]()
* [Crawling Server]()

A Case Study below can be used to illustrate how the various services and infrastructure can be used collectively to debug crawling when it goes wrong. And it does go wrong! 
...
* start by checking the [Registry Console]() to see if the crawl finished. 
* use the [CRAwling Monitor (CRAM)]() to check the job is still running. 
* check the [RabbitMQ (Queues)]() to see if messages are building up in some queues (in this example, imagine the dwca-validator queue is backed-up).
...
* SSH onto the [Crawling Server]() and quickly change to user "crap"
...
* use [Kibana]() to monitor crawling is moving through the various stages
