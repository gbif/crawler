 ...
 * configurations.
 */
public abstract class CrawlServerBaseService<T extends CrawlServerConfiguration> extends AbstractIdleService {
  ...
  protected void startUp() throws Exception {
    ...
    // Here we are starting as many queue listeners as specified in the configuration, each will process one queue
    // item at a time but they will all call the same QueueConsumer instance for this
    for (int i = 0; i < config.poolSize; i++) {
      QueueBuilder<UUID> builder = QueueBuilder.builder(curator, consumer, serializer, queuedCrawls);
  ...
  /**
   * Called once on startup to generate a consumer for new crawls.
   * The single consumer instance needs to be threadsafe.
   *
