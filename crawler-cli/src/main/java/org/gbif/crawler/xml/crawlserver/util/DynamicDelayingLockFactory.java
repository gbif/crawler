...
/**
 * A lock factory that forwards all calls to another Lock Factory but delays unlocking for a certain
 * amount of time. This can be used to make sure that we don't tax the resources of providers too
 * much. The delay depends on three factors but by default it is the amount of time that the lock
 * was taken out for. So if we held the lock for one second then we're going to delay unlocking for
 * another second. A minimum (default = 100ms and maximum (default = 2s) delay must be specified and
 * we're never going to delay less or more than those bounds.
 */
public class DynamicDelayingLockFactory extends ForwardingLockFactory {
  ...
  private class DynamicDelayingLock extends ForwardingLock {
    ...
    public void unlock() {
      ...
      long localDelay =
          stopwatch.elapsed(MILLISECONDS) > maxDelay ? maxDelay : stopwatch.elapsed(MILLISECONDS);
  ...
  /**
   * Default maximum value of milliseconds that unlock operations are delayed if no other value is
   * given.
   */
  public static final long DEFAULT_MAX_DELAY = 2000;

  /**
   * Default minimum value of milliseconds that unlock operations are delayed if no other value is
   * given.
   */
   ...
   * The exact delay depends on three factors: The time the lock was taken out (i.e. time between
   * acquiring the lock and releasing it), but it will always be between the provided minimum and
   * maximum delay.
   *
