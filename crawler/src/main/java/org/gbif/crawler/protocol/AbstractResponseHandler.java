 ...
 * <p/>
 * This class takes care of converting a {@link HttpResponse} into a byte[] which is then converted to a List of bytes.
 * You can get the underlying array back by using Guava: {@link Bytes#toArray(java.util.Collection)}}.
 * <p/>
 * A hash code is automatically calculated for the content of the response. The various getters throw an {@link
 * IllegalStateException} if the last response was not valid so check {@link #isValidState} before calling any of
 * those. An alternative indicator are the exceptions thrown by {@link #handleResponse(HttpResponse)}. If any is thrown
 * during processing the Response handler is in an illegal state until a successful response has been handled.
 * <p/>
...
public abstract class AbstractResponseHandler implements ResponseHandler<HttpResponse, List<Byte>> {
  ...
  /**
   * This needs to be implemented by subclasses to do actual processing on the XML data. This method is called for
   * every
   * single stream event (which includes whitespace etc.) so implementations need to check the event type and make sure
   * to work on the correct elements.
   *
   * @param reader to use
   *
   * @throws XMLStreamException if there was an error with the XML parsing (e.g. invalid XML)
   * @throws ProtocolException  if there is a problem with the (well-formed) XML response (e.g. missing attribute)
   */
  protected abstract void process(XMLStreamReader2 reader)
    throws XMLStreamException, FatalCrawlException, ProtocolException;

  /**
   * Needs to be implemented by subclasses to indicate to the parent class what the XML element is which delimits the
   * actual content of a result. Everything inside this element including the element itself is hashed.
   *
   ...
   *
   * @return the byte array corresponding to the response we received. Wrapped in a List of Bytes because this is
   *         extending a typed interface. This list is guaranteed to be convertable to a primitive {@code byte[]} using
   *         Guava's {@link Bytes#toArray(java.util.Collection)}}.
   *
  ...
  public final List<Byte> handleResponse(HttpResponse response)
    ...
    // The reader is used to parse the XML response and the writer is used to hash parts of the document
  ...
  protected static boolean isElement(XMLStreamReader reader, String name, boolean start) {
    return reader.getEventType() == (start ? XMLStreamConstants.START_ELEMENT : XMLStreamConstants.END_ELEMENT)
