...
/**
 * A context object used by {@link ScientificNameRangeStrategy} to encode lower and upper bounds for scientific name
 * ranges. At the moment either range has to be either absent/null or a three character string.
 */
// TODO: Make sure that lowerBound is "less than" upperBound
// TODO: We should probably just take a valid lower bound and calculate upper bound from there because we can't support
//       anything else anyway at the moment
public class ScientificNameRangeCrawlContext extends CrawlContext {

  /**
   * Some providers enforce a minimum length of search/filter terms. That's why we enforce it here as well. Three seems
  ...
  public final void setLowerBound(@Nullable String lowerBound) {
    Preconditions.checkArgument(lowerBound == null || lowerBound.length() == MIN_LENGTH,
  ...
  public final void setUpperBound(@Nullable String upperBound) {
    Preconditions.checkArgument(upperBound == null || upperBound.length() == MIN_LENGTH,
